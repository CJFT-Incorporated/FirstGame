...
TopDownGame.Game.prototype = {
	...
	var statsbut;
	panel.add(statsbut = new SlickUI.Element.Button(0,0, this.game.width, 80));
	statsbut.events.onInputUp.add(function () {console.log('Clicked button');});
	statsbut.add(new SlickUI.Element.Text(0,0, "Stats")).center();

	var crewbut;
	panel.add(crewbut = new SlickUI.Element.Button(0,80, this.game.width, 80));
	crewbut.events.onInputUp.add(function () {console.log('Clicked button');});
	crewbut.add(new SlickUI.Element.Text(0,0, "Crew")).center();

	var invbut;
	panel.add(invbut = new SlickUI.Element.Button(0,160, this.game.width, 80));
	invbut.events.onInputUp.add(function () {console.log('Clicked button');});
	invbut.add(new SlickUI.Element.Text(0,0, "Inventory")).center();
	
	var savebut;
	panel.add(savebut = new SlickUI.Element.Button(0,240, this.game.width, 80));
	savebut.events.onInputUp.add(function () {console.log('Clicked button');});
	savebut.add(new SlickUI.Element.Text(0,0, "Save")).center();
	
	var quitbut;
	panel.add(quitbut = new SlickUI.Element.Button(0,320, this.game.width, 80));
	quitbut.events.onInputUp.add(function () {console.log('Clicked button');});
	quitbut.add(new SlickUI.Element.Text(0,0, "Quit")).center();
  ...
  createFromTiledObject: function(element, group) {
	...
	triggerEncounter: function(player, encounterZone) {
		...
		rarity = Math.floor(Math.random()*256); //floor will be stored in an array of length 10.
		if (0 <= rarity && rarity <= 50)				//Each index in the array is increasingly less likely so index 9 ~ 1% chance to happen.
			...
			console.log("Bad math"); //Print statement for error
